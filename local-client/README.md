...
`npm run sync-content` refreshes generated runtime content into `local-client/app-content` (roles, docs, rooms, mojo, watch). The canonical app UI is authored directly at the repo-root `public/` tree and is not generated by the sync.

## Build The Installer

From `C:\Users\scott\Code\central\local-client`:
...
- MindShare Central UI from the repo-root `public/` tree, plus centralized roles, agents, catalogs, docs, phases, rooms, scripts, skills, templates, `AGENTS.md`, and `project-foundation.md`
...
The sync regenerates the app-content areas only; the canonical app UI at the repo-root `public/` tree is authored directly and left untouched by the sync. The script verifies paths are inside `local-client` and then copies fresh generated content while excluding Git data, local CLI state, node modules, cache folders, env files, logs, and common key/certificate files.
