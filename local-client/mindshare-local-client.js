...
async function loadConferenceRoomContext() {
  ...
  const fallbackOccupants = [
    ...
    { name: 'Vik', title: 'Program Architect' },
...
${providerRuntimeContext('codex', payload?.model)}
...
`;
  ...
  const codexModelArgs = payload?.model ? ['--model', payload.model] : [];
...
function resolveDeepSeekApiKey() {
  ...
  try {
    ...
    for (const line of raw.split(/\r?\n/)) {
      ...
      if (name !== 'DEEPSEEK_API_KEY' && name !== 'DEEP_SEEK_API_KEY' && name !== 'DEEPSEEK_KEY') continue;
...
async function connectDeepSeek(payload = {}) {
  ...
  if (!apiKey) {
    return {
      ok: false,
      action: 'config',
      message: 'No DeepSeek API key found. Add DEEPSEEK_API_KEY to your Central .env.local file.'
    };
  }
  if (payload?.checkOnly) {
    return {
...
async function getDeepSeekBalance() {
  ...
  if (!apiKey) {
    return {
      ok: false,
      action: 'config',
      error: 'No DeepSeek API key found. Add DEEPSEEK_API_KEY to your Central .env.local file.'
    };
  }
...
USER: ${message}`;
  ...
  if (!response.ok) {
    ...
    return {
      ...
      error: isAuthError
        ? 'DeepSeek API authentication failed. Check your DEEPSEEK_API_KEY in Central .env.local.'
        : errorText
    };
  }
...
async function reviewFileWithDeepSeek(file) {
  ...
  if (!apiKey) {
    return {
      ...
      findings: ['Add DEEPSEEK_API_KEY to your Central .env.local, then restart MindShare Central.'],
...
async function sendComboMessage(payload) {
  ...
  const claudePayload = await sendClaudeMessage({
    ...
    model: payload?.claudeModel || null
  });
...
`;
  ...
  const result = await run(launch.command, [...launch.argsPrefix,
    ...
    '--model',
    claudeModel
  ], { timeout: 900000, input: prompt, onChild: (child) => { activeChildren.set(sessionId, child); } });
...
function stopClaudeMessage(payload) {
  ...
  return { ok: true, stopped: Boolean(child) };
}
