...
const bundledPublicRoot = path.join(__dirname, '..', 'public');
...
function clearAutomationTimer() {
  // Vik automation was removed in the Central migration; no scheduled automations remain.
}
...
ipcMain.handle('mindshare:automation-control', async (_event, payload = {}) => {
  ...
  if (![].includes(role)) return { ok: false, error: `No automation timer is registered for ${role || 'that role'}.` };
