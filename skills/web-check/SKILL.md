---
name: web-check
description: Web Check skill owned by Liz (Website Manager). Runs on: obsidian change / weekly sweep. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/web-check/ to use.
---

# Web Check

Owned by **Liz (Website Manager)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `web-check/` folder into `~/.claude/skills/` and invoke it with `/web-check`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the web check capability for Liz (Website Manager). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: obsidian change / weekly sweep
- Invoke with `/web-check` or when the user describes the task this skill names.
...
3. Perform the web check work the owning role defines here.
...
A reviewable report or artifact for Liz (Website Manager). No silent state changes. Every gated action is surfaced for approval, not auto-applied.
