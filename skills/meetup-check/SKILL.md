---
name: meetup-check
description: Meetup Check skill owned by Jay (Meetup Coordinator). Runs on: daily. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/meetup-check/ to use.
---

# Meetup Check

Owned by **Jay (Meetup Coordinator)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `meetup-check/` folder into `~/.claude/skills/` and invoke it with `/meetup-check`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the meetup check capability for Jay (Meetup Coordinator). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: daily
- Invoke with `/meetup-check` or when the user describes the task this skill names.
...
3. Perform the meetup check work the owning role defines here.
...
A reviewable report or artifact for Jay (Meetup Coordinator). No silent state changes. Every gated action is surfaced for approval, not auto-applied.
...
> Reads Meetup + emails Scott. Needs Meetup + email creds.
