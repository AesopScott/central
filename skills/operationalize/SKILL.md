---
name: operationalize
description: Operationalize skill owned by Cole (Hr Director). Runs on: recruiting central. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/operationalize/ to use.
---

# Operationalize

Owned by **Cole (Hr Director)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `operationalize/` folder into `~/.claude/skills/` and invoke it with `/operationalize`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the operationalize capability for Cole (Hr Director). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: recruiting central
- Invoke with `/operationalize` or when the user describes the task this skill names.
...
3. Perform the operationalize work the owning role defines here.
...
> Cole-owned. Repairs role files from git/backup.
