...
description: Morgan Ops Review skill owned by Morgan (Operations Director). Runs on: new operational process. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/morgan-ops-review/ to use.
---

# Morgan Ops Review

Owned by **Morgan (Operations Director)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `morgan-ops-review/` folder into `~/.claude/skills/` and invoke it with `/morgan-ops-review`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the morgan ops review capability for Morgan (Operations Director). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: new operational process
- Invoke with `/morgan-ops-review` or when the user describes the task this skill names.
...
3. Perform the morgan ops review work the owning role defines here.
...
A reviewable report or artifact for Morgan (Operations Director). No silent state changes. Every gated action is surfaced for approval, not auto-applied.
