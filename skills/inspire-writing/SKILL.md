...
description: Inspire Writing skill owned by June (Staff Writer). Runs on: daily reminder. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/inspire-writing/ to use.
---

# Inspire Writing

Owned by **June (Staff Writer)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `inspire-writing/` folder into `~/.claude/skills/` and invoke it with `/inspire-writing`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the inspire writing capability for June (Staff Writer). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: daily reminder
- Invoke with `/inspire-writing` or when the user describes the task this skill names.
...
3. Perform the inspire writing work the owning role defines here.
...
A reviewable report or artifact for June (Staff Writer). No silent state changes. Every gated action is surfaced for approval, not auto-applied.
