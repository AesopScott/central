---
name: agentify
description: Agentify skill owned by Tess (Autonomy Engineer). Runs on: autonomy central tool. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/agentify/ to use.
---

# Agentify

Owned by **Tess (Autonomy Engineer)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `agentify/` folder into `~/.claude/skills/` and invoke it with `/agentify`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the agentify capability for Tess (Autonomy Engineer). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: autonomy central tool
- Invoke with `/agentify` or when the user describes the task this skill names.
...
3. Perform the agentify work the owning role defines here.
...
> Builds autonomy baselines — Tess/Vik owner-gated.
