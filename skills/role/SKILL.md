---
name: role
description: Role skill owned by Ana (Recruiter). Runs on: on new role request. Scaffold generated from the Mindshare skills registry; install into ~/.claude/skills/role/ to use.
---

# Role

Owned by **Ana (Recruiter)**. Generated scaffold from the Mindshare skills registry (`roles/skills-registry.json`). This file is a real, installable Claude skill: drop the `role/` folder into `~/.claude/skills/` and invoke it with `/role`. The structure below is honest scaffold depth — the owning role deepens the body with its real procedure.

## Purpose

Provide the role capability for Ana (Recruiter). Use this skill when the owning role needs a repeatable, registered procedure rather than an ad-hoc prompt.

## When To Use

- Trigger: on new role request
- Invoke with `/role` or when the user describes the task this skill names.
...
3. Perform the role work the owning role defines here.
...
A reviewable report or artifact for Ana (Recruiter). No silent state changes. Every gated action is surfaced for approval, not auto-applied.
...
> Skill already exists live. Modify, do not create new.
